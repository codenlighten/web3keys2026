...
class SqliteStorage {
  ...
  async removeOutput(txid, vout) {
    const info = this.db.prepare('DELETE FROM outputs WHERE txid = ? AND vout = ?').run(txid, vout);
    return info.changes > 0;
  }
  ...
  async listCertificates({ certifiers, types } = {}) {
    ...
    if (certifiers && certifiers.length)
      rows = rows.filter((c) => certifiers.includes(c.certifier));
  ...
  async getPendingAction(reference) {
    const row = this.db
      .prepare('SELECT record FROM pending_actions WHERE reference = ?')
      .get(reference);
    return row ? JSON.parse(row.record) : null;
  }

  async removePendingAction(reference) {
    const info = this.db.prepare('DELETE FROM pending_actions WHERE reference = ?').run(reference);
    return info.changes > 0;
  }
