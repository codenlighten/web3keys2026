...
test('BRC-100 encrypt/decrypt, hmac, signature round trips', async () => {
  ...
  assert.ok(
    (await b.verifyHmac({ data: 'payload', hmac: h.hmac, protocolID: proto, keyID: '2' })).valid
  );
  assert.ok(
    !(await b.verifyHmac({ data: 'tampered', hmac: h.hmac, protocolID: proto, keyID: '2' })).valid
  );

  const s = await b.createSignature({ data: 'sign me', protocolID: proto, keyID: '3' });
  assert.ok(
    (
      await b.verifySignature({
        data: 'sign me',
...
test('BRC-100 certificate acquire / prove / relinquish', async () => {
  ...
  const b = wallet.brc100;
  const cert = (
    await b.acquireCertificate({
      type: 'KYC',
      certifier: '02abc',
      fields: { name: 'Greg', age: '30' },
    })
  ).certificate;
  assert.equal((await b.listCertificates()).totalCertificates, 1);
  const proof = await b.proveCertificate({
    certificate: cert,
    fieldsToReveal: ['name'],
    verifier: wallet.identity.identityKey,
  });
...
test('createAction funds, signs and broadcasts via BRC-100', async () => {
  ...
  const res = await wallet.brc100.createAction({
    ...
    outputs: [
      { lockingScript, satoshis: 10000, basket: 'payments', outputDescription: 'test pay' },
    ],
    labels: ['test'],
  });
  assert.ok(res.txid);
