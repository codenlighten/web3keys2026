...
test('SqliteStorage implements the full WalletStorage surface', async () => {
  ...
  // outputs / baskets
  await storage.insertOutput({ basket: 'ord', txid: 'aa', vout: 0, satoshis: 1, lockingScript: '00', tags: ['x'] });
  await storage.insertOutput({ basket: 'pay', txid: 'bb', vout: 1, satoshis: 100, lockingScript: '01' });
  ...
  // certificates
  await storage.insertCertificate({ serialNumber: 's1', type: 'KYC', certifier: 'c1', fields: { name: 'g' } });
  ...
  // actions
  await storage.insertAction({ txid: 't1', description: 'd', status: 'unproven', labels: ['l1'], fee: 5 });
