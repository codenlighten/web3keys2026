...
router.post('/api/auth/register', authLimiter, h(async (req, res) => {
  const { email, password } = req.body || {};
  const result = await svc.register({ email, password });
  res.status(201).json(result);
}));

router.post('/api/auth/verify', authLimiter, h(async (req, res) => {
  const { email, code } = req.body || {};
  const profile = svc.verifyRegistration({ email, code });
  res.json({ verified: true, profile });
}));

router.post('/api/auth/resend', authLimiter, h(async (req, res) => {
...
router.post('/api/auth/login', authLimiter, h(async (req, res) => {
  const { email, password } = req.body || {};
...
router.get('/api/wallet/balance', authed, h(async (req, res) => {
  res.json(await svc.getBalance(req.user));
}));

router.post('/api/wallet/send', authed, h(async (req, res) => {
